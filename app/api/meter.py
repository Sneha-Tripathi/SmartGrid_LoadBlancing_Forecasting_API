...
@router.post("/", response_model=SmartMeterResponse, status_code=status.HTTP_201_CREATED)
def create_meter(meter: SmartMeterCreate, db: Session = Depends(get_db)):
...
@router.put("/{meter_id}", response_model=SmartMeterResponse)
def update_meter(
    meter_id: int,
