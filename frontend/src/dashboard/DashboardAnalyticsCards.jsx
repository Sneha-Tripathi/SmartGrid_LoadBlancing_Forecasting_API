...
const ANALYTICS_CARDS_CONFIG = [
  ...
  { key: "carbon_saved", title: "Carbon Saved", icon: FaLeaf, gradient: "from-green-500 to-emerald-600", unit: "tons CO\u2082" },
...
const DashboardAnalyticsCards = memo(function DashboardAnalyticsCards() {
  const { data, loading } = useApi(() => analyticsService.getTodayEnergy(), {
    onError: () => console.warn("Analytics API unavailable"),
  });
  ...
  if (loading && !source) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5 mt-8">
        {ANALYTICS_CARDS_CONFIG.map((_, idx) => (
          <CardLoader key={idx} delay={idx * 100} />
        ))}
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5 mt-8">
      {ANALYTICS_CARDS_CONFIG.map((card, index) => {
        ...
        return (
          <div
            key={card.key}
            className="relative group overflow-hidden bg-[#101827] border border-slate-800 rounded-2xl p-5 hover:border-cyan-500/30 transition-all duration-500 hover:scale-[1.02] hover:shadow-xl hover:shadow-cyan-500/5"
          >
