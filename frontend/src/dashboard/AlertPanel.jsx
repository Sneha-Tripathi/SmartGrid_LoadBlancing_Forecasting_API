...
const AlertPanel = memo(function AlertPanel() {
  ...
  return (
    <div className="bg-[var(--card)] border border-slate-800 rounded-2xl p-6 h-full">
      <h2 className="text-xl font-semibold text-white mb-6">Live Alerts</h2>
      <div className="space-y-4">
        {alerts.length === 0 ? (
          ...
          alerts.map((alert) => (
            <div
              key={alert.id}
              className={`border-l-4 ${getBorder(alert.status)} bg-slate-900 rounded-xl p-4`}
            >
              <div className="flex items-start gap-3">
                {getIcon(alert.status)}
                <div className="flex-1">
                  <div className="flex justify-between">
                    <h3 className="text-white font-semibold">{alert.status}</h3>
